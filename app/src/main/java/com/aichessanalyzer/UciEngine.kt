...
class UciEngine(private val context: Context) {
    ...
    private suspend fun ensure(): Boolean = withContext(Dispatchers.IO) {
        ...
        val dir = File(context.filesDir, "engine").apply {
            mkdirs()
        }
        ...
        if (!exe.exists()) {
            context.assets
                .open("stockfish/arm64-v8a/stockfish")
                .use { input ->
        ...
        p = ProcessBuilder(exe.absolutePath)
            .redirectErrorStream(true)
            .start()

        w = p!!.outputStream.bufferedWriter()
        r = p!!.inputStream.bufferedReader()

        send("uci")
        waitToken("uciok", 5000)
    ...
    private fun waitToken(token: String, ms: Long) {
        ...
        while (System.currentTimeMillis() - t < ms) {
            if (r?.ready() == true) {
                if (r!!.readLine()?.startsWith(token) == true) {
                    return
                }
            } else {
                Thread.sleep(10)
    ...
    suspend fun analyze(
        fen: String,
        depth: Int
    ): EngineInfo = withContext(Dispatchers.IO) {

        if (!ensure()) {
            return@withContext EngineInfo()
        }

        send("isready")
        waitToken("readyok", 5000)
    ...
    private fun parse(
        ...
        val a = s.split(" ")
        val d = a.indexOf("depth")
        val sc = a.indexOf("score")
        val pv = a.indexOf("pv")
        ...
        if (sc >= 0 && sc + 2 < a.size) {
            if (a[sc + 1] == "cp") {
                cp = a[sc + 2].toIntOrNull()
                mate = null
            } else if (a[sc + 1] == "mate") {
                mate = a[sc + 2].toIntOrNull()
                cp = null
            }
        }
        ...
        return old.copy(
            depth = if (d >= 0) {
                a.getOrNull(d + 1)?.toIntOrNull() ?: old.depth
            } else {
                old.depth
            },
            cp = cp,
